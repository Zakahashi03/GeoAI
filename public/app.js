...
// Kartoppsett
const map = L.map('map').setView([60.472, 8.4689], 5); // Sørg for at ID-en matcher
...
// Funksjon for å håndtere flomfare
function håndterFlomfare() {
  const knapp = document.getElementById("flomfareKnapp");
  ...
  knapp.addEventListener("click", async () => {
    if (!flomfareVises) {
      ...
      knapp.innerHTML = `Skjul flomfare<span> Klikk for å skjule flomfareområdene i Agder</span>`;
      flomfareVises = true;
    } else {
      flomfareLag.forEach(layer => map.removeLayer(layer));
      flomfareLag = [];
      knapp.innerHTML = `Vis flomfare<span> Klikk for å vise flomfareområdene i Agder</span>`;
      flomfareVises = false;
    }
  });
}
...
function flyttKnapperTilOmKartet() {
  ...
  const buttons = [
    ...
    document.getElementById("flomfareKnapp"),
